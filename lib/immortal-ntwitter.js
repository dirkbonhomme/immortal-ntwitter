...
ImmortalNTwitter.prototype.immortalStream = function(method, params, callback) {
  ...
  {
	// we need an up-to-date parameter list to resurrect, otherwise, we can 't add users to control streams transparently
	if (typeof params === 'function') {
		var streamParameters = params();
	} else {
		var streamParameters = params;
	}
	
    self.stream(method, streamParameters, function(stream){
      stream
      ...
      .on('missedHeartbeat', function() {
      	immortalStream.emit('missedHeartbeat');
      });
      stream.checkHeartbeat(180000);
    });
  }
